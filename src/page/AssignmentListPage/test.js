export const TEST_CODE = `#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <signal.h>
...
#define PARSE_SCHED(buf, policy)       
    if (strcmp(buf, "FIFO") == 0)      
        policy = FIFO;                 
    else if (strcmp(buf, "RR") == 0)   
        policy = RR;                   
    else if (strcmp(buf, "PRIO") == 0) 
        policy = PRIO;                 
...
{
    printf("----n__preemptive_workern----n");
    while (1)
        ;
    printf("----n__preemptive_worker_endn----n");
}
...
{
    printf("----n__non_preemptive_workern----n");
    for (int i = 0; i < 100000; i++)
    {
    }
    printf("----n__non_preemptive_worker_endn----n");
}
